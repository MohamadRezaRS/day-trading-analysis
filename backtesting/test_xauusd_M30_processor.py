...
class XAUUSD_M30_Processor:
    ...
    def _load_models_and_scalers(self):
        for pattern in ["bullish orderblock", "bearish orderblock","bearish fvg"]:
            model_path = self.model_paths[pattern]
            scaler_path = self.scaler_paths[pattern]

            if os.path.exists(model_path):
                self.models[pattern] = joblib.load(model_path)
            if os.path.exists(scaler_path):
                self.scalers[pattern] = joblib.load(scaler_path)
    ...
    def _process_trigger(self, candles, pattern, noisy_day, is_highest_day, is_highest_week, session_code, direction):
        ...
        if direction == "bullish":
            vector = {"noisy_day" : int(noisy_day),"is_highest_day": int(is_highest_day)
                      , "is_highest_week":int(is_highest_week), "volume":scaled_volume, "session":int(session_code)}
        else:
            vector = {"noisy_day":int(noisy_day),"is_highest_day": int(is_highest_day)
                      ,"is_highest_week": int(is_highest_week),"volume": scaled_volume
                      , "session":int(session_code),"weekday": int(weekday)}
